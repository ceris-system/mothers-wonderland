...
async function handleAction(action) {
    // forgotPassword uses its own username field (forgotUserInput) since
    // the person may open "Forgot password?" without having typed
    // anything into the main login username field yet.
    const user = action === 'forgotPassword'
    ...
    try {
        ...
        if (data.success || data.status === "REQUIRE_UPDATE") {
            ...
            if (action === 'forgotPassword') {
                // A successful reset should NOT auto-login — the identity
                // check here (Department/Client name) is lighter than a
                // real password, so we send them back to the normal login
                // form to sign in with their new password instead.
                showModal("PASSWORD RESET", data.message || "Password updated successfully. You can now log in.", "success");
                hideForgotPasswordSection();
                const userField = document.getElementById('userInput');
                if (userField) userField.value = user;
            }
            else if (status === "ACTIVE" || status === "SUCCESS" || action === 'updatePassword') {
            ...
            const failTitle = action === 'forgotPassword' ? "RESET FAILED" : "ACCESS DENIED";
            showModal(failTitle, data.message || "Invalid credentials.", "error");
        }
    } catch (error) {
